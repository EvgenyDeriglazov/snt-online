...
class TargetPaymentsPage(TestCase):
    """MembershipPaymentsPage in Membership app view."""
    fixtures = ['test_db.json']

    # Test URL access
    def test_target_page_url_by_unauthenticated_user(self):
        """"""
        response = self.client.get('/plot-id-1/target/')
        self.assertEqual(response.status_code, 302)

    def test_target_page_url_by_super_user(self):
        """"""
        self.client.login(username='admin', password='admin')
        response = self.client.get('/plot-id-1/target/')
        self.assertEqual(response.status_code, 404)
    
    def test_target_page_url_by_chairman_user(self):
        """"""
        self.client.login(username='chairman2', password='pswd2000')
        response = self.client.get('/plot-id-1/target/')
        self.assertEqual(response.status_code, 404)

    def test_target_page_url_by_accountant_user(self):
        """"""
        self.client.login(username='accountant2', password='pswd4000')
        response = self.client.get('/plot-id-1/target/')
        self.assertEqual(response.status_code, 404)

    def test_target_page_url_by_true_owner_user(self):
        """Test owner access to his target data."""
        self.client.login(username='owner1', password='pswd5000')
        response = self.client.get('/plot-id-1/target/')
        self.assertEqual(response.status_code, 200)

    def test_target_page_url_by_fake_owner_user(self):
        """Test owner access to other owner target data."""
        self.client.login(username='owner2', password='pswd6000')
        response = self.client.get('/plot-id-1/target/')
        self.assertEqual(response.status_code, 404)

    # Test URLconf name and template
    def test_target_page_url_conf_name(self):
        """"""
        self.client.login(username='owner1', password='pswd5000')
        response = self.client.get(reverse('target', kwargs={'plot_id': 1}))
        self.assertEqual(response.status_code, 200)

    def test_target_page_template(self):
        """"""
        self.client.login(username='owner1', password='pswd5000')
        response = self.client.get('/plot-id-1/target/')
        self.assertTemplateUsed(response, 'target_payments_page.html')

    # Test context content and data
    def test_target_page_context_keys(self):
    ...
    def test_target_page_context_data(self):
        ...
        payment_data_list = TPayment.objects.filter(
